...
class StoreTodo {
	...
	constructor() {
		this._id = 0;
		this._key = "TODOS";
		this._storeTodo$ = new BehaviorSubject<ITodo[]>(this.getFromLocalStorage());
	}
	...
	public changeCompleted(id: number): void {
	...
	public deleteTodo(id: number): void {
		const todos: ITodo[] = this.todos().filter((todo) => todo.id !== id);
		this._storeTodo$.next([...todos]);
		this.saveInLocalStorage();
	}
